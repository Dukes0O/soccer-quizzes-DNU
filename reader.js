...
(function () {
  ...
  function rewriteLinks(slug, book, baseIsReadme) {
  ...
  function renderBookHome(slug) {
    return getBook(slug).then(function (book) {
      return fetchMD(book._path + '/README.md').then(function (md) {
        ...
        rewriteLinks(slug, book, true);
        window.scrollTo(0, 0);
      });
    });
  }

  function renderChapter(slug, file) {
    return getBook(slug).then(function (book) {
  ...
  function route() {
    ...
    p.then(function () {
      ...
      if (parts.length === 1) return renderBookHome(parts[0]);
      return renderChapter(parts[0], parts[1]);
    }).catch(function (err) {
